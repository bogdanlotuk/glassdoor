...
public class Page {
    ...
    public void getLinksFromLists(){
        if(($$x(LIST_JOB_LINK).size() > 0) || modalCloseButton.isDisplayed()) {
            closeModal();
            //Load all available jobs
            String lastJobLink = $x(LIST_JOB_LINK_LAST_CHILD).attr("href").replaceAll(",","");
            String currentLastJobLink = "";

            while($x(BUTTON_SHOW_MORE_JOBS).isDisplayed() && !lastJobLink.equals(currentLastJobLink)){
                $x(BUTTON_SHOW_MORE_JOBS).click();
                closeModal();
                lastJobLink = currentLastJobLink;
                currentLastJobLink = $x(LIST_JOB_LINK_LAST_CHILD).attr("href").replaceAll(",","");
    ...
    public void collectInfoFromTheLink(){
        for(String link : linksFromList){
            open(link);
            if($x(BUTTON_SHOW_MORE).exists()){
                $x(BUTTON_SHOW_MORE).click();
            }
    ...
    public void selectDatePosted(){
        ...
        sleep(1000);
        SelenideElement selectPostedDays = $x("//div[contains(text(), '" + selectDays + "')]/..");
        if(selectPostedDays.exists()) {
